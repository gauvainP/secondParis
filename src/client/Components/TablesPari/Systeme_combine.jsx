...
export default class Systeme_combine extends Component {
    constructor(props) {
        super(props);
        this.state = {
...
gainCombine += tabIncr[j];
 ...
 gain = 0;
for (var j = 0; j < teams.index.length; j++) {
    gain += teams.cote[teams.index[j]] * mise;
...
const nbParis= eval(this.props.nbParis)+ eval(1);
        var n = 0;
        ...
        return (
            <div className="home">
                ...
                <table border="1">
                    <caption>Systeme {systemeChoice}/ {nbParis-1} + Combiné </caption>
                    <thead>
                        <tr>
                            <th>Vainqueur(s)</th>
                            <th>Gains</th>
                    ...
                    <tbody>
...

                        {
                            coup.map((generalArray, index) => (
                                ...
                                <tr key={index} style={this.showOnlyPositive((this.calculGain(generalArray, teams, mise) - (nbParis*mise)), isOnlyPositive) ? { display: 'none' } : console.log()} >
                                    <td >
                                    ...
                                    <td style={this.colorCase((this.calculGain(generalArray, teams, mise) - (nbParis*mise)) ) ? { backgroundColor: 'green' } : { backgroundColor: 'red' }} >
                                            {(this.calculGain(generalArray, teams, mise) - (nbParis*mise)).toFixed(2) }  </td>
